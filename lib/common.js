...
const coordinatesToPosition = (coordinates, side) => {
  const horizontal = (x) => side === "top" ? x : 2 - x
  const vertical = (y) => side === "top" ? y : 4 - y
  switch (coordinates.join(' ')) {
    case [1, vertical(0)].join(' '):
      return 'GK'
    case [horizontal(0), vertical(1)].join(' '):
      return 'RB'
    case [horizontal(1), vertical(1)].join(' '):
      return 'CB'
    case [horizontal(2), vertical(1)].join(' '):
      return 'LB'
    case [horizontal(0), vertical(2)].join(' '):
      return 'RM'
    case [horizontal(1), vertical(2)].join(' '):
      return 'CM'
    case [horizontal(2), vertical(2)].join(' '):
      return 'LM'
    case [horizontal(0), vertical(3)].join(' '):
      return 'RWF'
    case [horizontal(1), vertical(3)].join(' '):
      return 'ST'
    case [horizontal(2), vertical(3)].join(' '):
      return 'LWF'
  }
}
